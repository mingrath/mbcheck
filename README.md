...
A pre-purchase inspection for second-hand Apple-silicon MacBooks — M1 through M5, Air and
Pro. You run one command at the shop, answer some questions about things a script cannot
feel or hear, and walk out with a report.

It is built for Thailand: the repair prices are in baht and dated, the phrases are Thai, and
one section covers Thai law. The checks themselves assume no country.

---

## Run it

On the machine you are thinking of buying, open Terminal (`Cmd + Space`, type `terminal`)
and paste:
...
That is the whole procedure. About 12–15 minutes.

> **Not `curl … | bash`.** Piping into bash hands the script's standard input to the pipe, so
> every question it asks you reads nothing and answers itself. Process substitution keeps
> your keyboard attached. It works in both `zsh` and `bash`, so it works on every Mac in
> scope.
...
guide's own rules, refusing one *method* costs nothing; refusing every method for a fact is
itself a finding.

### What it will and will not do to the machine
...
```
--stolen-check   Also look the serial up on stolenregister.com.
                 OFF by default, deliberately: it sends someone else's serial
                 to a third party, from that person's own machine, before you
                 have bought it. The script prints the URL so you can run it
                 on your own phone instead.
...
Three kinds of risk, in the order they can hurt you.

### 1. Software locks — can you actually use it?

This is where a MacBook becomes worthless rather than merely faulty, and it is the part
most checklists get wrong.

- **Activation Lock.** The script reads it directly. **`Enabled` is the normal reading on an
  honest machine** — it reports whether Find My is on right now, which before the seller
  signs out it should be. Advice to "walk away if Activation Lock is enabled" would fail
  almost every honest seller. What matters is that it reads `Disabled` *after* they sign out
  in front of you.
- **Device management (MDM / DEP).** Read in one unprivileged command. In Thailand this is
  as often an instalment or pawn-shop lock (**ติดผ่อน**) as a corporate one.
- **Automated Device Enrollment.** The one that actually bricks machines — and **the one
  nothing can detect.** See [what this cannot see](#what-the-script-cannot-see).
- **Recovery Lock.** A Mac can boot macOS perfectly and be permanently unable to enter
  recovery. Nothing on a running system exposes it. Only a reboot finds it, which is why
  that is one of the prompts.
- **Boot policy.** If SIP is off, the machine has been deliberately downgraded to Permissive
  Security — which also *hides the repair-history pane entirely*. The script checks this
  first, because everything below depends on it.

### 2. Provenance — is it really theirs to sell?

Nothing on an M1–M5 Mac reveals where it was sold, and serials have been randomised since
2021. So provenance is tested by paperwork and by contradiction: the script captures what
the seller *claims* before it opens the pane that can contradict them. A claim that does not
survive contact with the machine is a finding about the seller.

One contradicted claim is a misunderstanding. **Two is a pattern** — and the biggest risk on
this whole list is one you cannot check at all, where the seller's word is your only defence.
A seller caught out twice has spent that word.

### 3. Condition — is it what was advertised?

Battery, thermals under load, ports, screen, keyboard, sound. RAM and storage are soldered on
M1–M5, so a "16GB" machine cannot have been quietly downgraded — which narrows this a lot
compared with the Intel era.

Cosmetic condition is deliberately **not** covered. You can see scratches yourself and do not
need a procedure for it.
...
```
Is it remediable by you, at any price?
  NO  → 🛑 HIGH RISK
  YES ↓
Does fixing it cost real money?
  YES → 💰 RENEGOTIATE   (with a dated baht figure)
  NO  → 📝 NOTE
```

**One override.** A fault that is cheap to fix but **spreads** is promoted to 🛑 regardless of
price, because a quote prices today's damage and not what it takes with it. The list is
short, closed and named, so it never becomes a judgement call in a shop:

- swollen battery (lifts the trackpad, can crack the display, fire risk)
- liquid damage / corrosion
- shutdown or reboot under sustained load

**No finding stops the run.** The script always finishes and always prints the whole report.
You decide at the end, with everything in front of you, instead of walking out on the first
bad line.

**There is a fourth outcome, and it is the important one:** `??` **not established**. A check
that could not be run is *not* a check that passed. Every prompt accepts "not sure", and
"not sure" is recorded as exactly that. The most dangerous report is one that reads clean
because nobody looked.

**On price:** every 💰 carries a real Thai repair price with a date, so a stale number is
visibly stale. The report tallies them and shows the total. It takes **no position** on when
a total is too much — a bargain hunter and someone buying a daily driver have genuinely
different ceilings, and inventing a threshold would be false precision.

Resale impact is deliberately **not** in the tally. It is not a repair cost, and adding it
would double-count against the prices already there.

---

## What the script cannot see

Read this part. A tool that hides its own blind spots is worse than no tool.

### It cannot tell you whether the serial is in a company's Apple Business Manager

This is the single most serious gap, and it is not fixable at any privilege level. No
command — with or without `sudo` — can ask Apple whether a serial belongs to an organisation.

`Enrolled via DEP: No` means only *"not enrolled right now, on this OS install."* A serial can
sit in an organisation's inventory unassigned today and be assigned tomorrow, with one click
by an admin who never touches the machine. On macOS 14 and later the Mac is then walled off:
a full-screen enrollment screen you can dismiss **once**, for eight hours.

Erasing does not help — the registration is server-side, against the serial. And over
**Ethernet** a managed Mac can re-enrol *silently, with no screen at all*, which is exactly
why the erase test in the script insists on **Wi-Fi**.

The best available test is watching the seller erase the machine in front of you. Even a
clean pass proves only "not assigned today". That residual risk is irreducible. Price it.

### It cannot measure SSD wear

`S.M.A.R.T. status: Verified` is a pass/fail bit, not a health figure — do not let it imply a
healthy drive. The wear counter genuinely exists on the drive and is readable without
privileges; macOS simply ships no tool that opens it. The tools that would (`smartctl`, or a
compiler) mean installing software on a stranger's machine, which this script will not do.

### It cannot tell whether the screen or the top case were replaced

Parts & Service History tracks **four** things: logic board, Touch ID board, lid angle sensor
(M5 only), and display (**MacBook Neo only**). On every MacBook Air and Pro the display is
**not** tracked. Neither is the battery, top case, speaker, camera, microphone, fan, trackpad
or Thunderbolt board.

So a machine with a swapped screen, battery, keyboard and speakers shows a perfectly clean
pane. A clean pane means *no tracked part was flagged* — never *never repaired*.

> Older advice says to check True Tone to catch a swapped panel. **That test is dead.** Apple
> never documented it for Mac, and Repair Assistant now restores display calibration on Air
> and Pro 2020–2026 — so it fails in both directions. It is not in the script.

### It cannot test the RAM

Only Apple Diagnostics really can, and that needs a full shutdown, so a script cannot run it.
It also takes about ten minutes plus a restart, which does not fit a shop visit.

### It cannot read anything from before the last erase

Panic reports and diagnostic logs live on the data volume and do not survive an erase. What
*does* survive is a dated receipt *of* the erase, which the script reads — you cannot recover
the history, but you can see how far back any history could possibly reach.

### It cannot see repair history at all on macOS before Tahoe 26

The pane requires macOS 26, and there is no command-line equivalent. On an older machine the
script reports this as **could not look**, not as a fault — the seller is refusing nothing,
the guide simply has no method to offer. Asking a stranger to spend forty minutes updating a
machine you do not own is not a check that fits in a shop visit.

What you can still do costs nothing: get **ไม่เคยแกะ ไม่เคยซ่อม** (never opened, never
repaired) said in writing in chat. Only 2.7% of Thai listings claim it.

---

## Model appendix

The script resolves all of this itself. It is here so you can check its work.

### Ports — there is no single right answer, and this is where old checklists mislead

"All three Thunderbolt ports must charge" is **wrong for half the machines in scope.**

| Model | USB-C | MagSafe 3 |
|---|---|---|
| MacBook Air 13" M1 (2020) | 2 | **No** |
| MacBook Air 13"/15", M2–M5 | 2 | Yes |
| MacBook Pro 13" M1 (2020), M2 (2022) | 2 | **No** |
| MacBook Pro 14" base M3 (Nov 2023) | **2** | Yes |
| MacBook Pro 14"/16", all other Apple silicon | 3 | Yes |

A Mac charges over **one port at a time**, using whichever source supplies the most power —
so plugging into several at once proves nothing. Test them one at a time.

### Fans

Every **MacBook Air M1–M5 is fanless**. Silence under load is correct behaviour, not a fault,
and "listen for the fans" is meaningless on half the machines here. The base-M3 14" Pro has
**one** fan where its Pro/Max siblings have two.

What replaces the fan check on an Air is the thermal *trajectory*: a healthy machine climbs
and then flattens.

### Battery cycles

**Apple rates every Apple-silicon MacBook at 1000 cycles.** The widely repeated "over 300
cycles, renegotiate" is an Intel-era number with no Apple basis, and applying it to an M-series
machine will lose you good machines.

macOS reports two conditions, not the old four: **Normal** and **Service Recommended**.

### Defects and service programmes

**No Apple service programme, recall or repair extension has ever covered any Apple-silicon
MacBook.** Checked against Apple's live index plus archived snapshots from 2022 to 2026. There
is no butterfly keyboard here, no flexgate, and no staingate. So the appendix is a
*configuration* table, not a defect list.

### macOS support

No Apple-silicon Mac has ever been dropped from a macOS release; macOS 27 still supports the
2020 M1. **Age alone is never a reason to decline any M1–M5 MacBook.** One dated cliff exists
and it is a note, not a discount: macOS 27 is the last release with full Rosetta 2 (~autumn
2027), and it hits an M1 and an M5 identically.

---

## Thailand — prices, phrases, market risks

### Repair prices

All observed **2026-08-04**. Apple figures come from Apple Thailand's own published service
pricing; independent figures are shop-published ranges. Apple's price does not bind authorised
providers, who set their own fees.

| Repair | Apple TH | Independent |
|---|---|---|
| Battery — Air 13" M1–M3 | ฿5,590 | ฿2,800–4,800 |
| Battery — Air 13" M4/M5 | ฿6,290 | — |
| Battery — Air 15" M2–M5 | ฿6,990 | ฿4,590–5,500 |
| Battery — Pro 14" M5 | ฿7,990 | — |
| Battery — all other Pro | ฿8,690 | ฿3,500–6,800 |
| Display — Air | not published | ฿10,300–17,900 |
| Display — Pro 14"/16" mini-LED | not published | ฿17,990–18,990 |
| Port that will not charge | not published | ฿1,500–3,500 |
| Top case / keyboard — Air 13" | ฿6,190 + ฿1,605 | ฿3,500–5,500 |
| Top case / keyboard — Air 15" | ฿7,490 + ฿1,605 | — |
| Top case / keyboard — Pro | ฿9,490 + ฿1,605 | — |
| Speakers (pair) | — | ฿4,000–4,500 |
| Fan clean and re-paste | — | ฿1,000–2,000 |
| Charger 140W (Pro 16") | ฿3,190 | ฿2,890–2,990 |
| MagSafe 3 cable — needed separately | ฿1,590 | ฿1,590 |

Two things worth knowing before you quote any of these:

- **Every independent shop found fits aftermarket cells, not genuine Apple batteries.** Any
  sub-฿8,690 claim of a "genuine" battery should be treated as unverified.
- **A disclosed battery replacement prices a machine *up* in the Thai listing market**, not
  down. It is an advertised selling point. Do not treat it as a deduction.

### Phrases

Written for a male speaker (**ครับ**); substitute **ค่ะ** throughout if you are female.

| English | Thai |
|---|---|
| Can I check the machine first? | ขอเช็คเครื่องก่อนได้ไหมครับ |
| Is it a Thai-market unit or an overseas one? | เครื่องศูนย์ไทยหรือเครื่องหิ้วครับ |
| Please sign out of iCloud in front of me | รบกวนลงชื่อออกจาก iCloud ต่อหน้าผมเลยได้ไหมครับ |
| Is the machine still attached to iCloud? | เครื่องยังติดไอคลาวด์อยู่ไหมครับ |
| Is it still on an instalment plan, or on MDM? | เครื่องยังติดผ่อนหรือติด MDM อยู่ไหมครับ |
| Was it ever a company or school machine? | เครื่องนี้เคยลงทะเบียน MDM หรือเป็นเครื่องบริษัท/เครื่องโรงเรียนไหมครับ |
| After a reset, does it show "Remote Management"? | ถ้ารีเซ็ตเครื่องแล้ว มันขึ้นหน้า Remote Management ไหมครับ |
| How many charge cycles? | แบตเตอรี่กี่รอบชาร์จครับ |
| What is the battery health? | สุขภาพแบตเตอรี่เท่าไหร่ครับ |
| Ever repaired, or had any part replaced? | เครื่องเคยซ่อมหรือเปลี่ยนอะไหล่อะไรบ้างไหมครับ |
| Has it ever been opened? | เครื่องเคยแกะไหมครับ |
| Receipt with the serial number on it? | ขอใบเสร็จรับเงินที่มีหมายเลขเครื่อง (Serial Number) ระบุไว้ด้วยได้ไหมครับ |
| May I see the original purchase receipt? | ขอดูใบเสร็จตอนซื้อเครื่องได้ไหมครับ |
| Your name and phone number on the receipt? | ขอชื่อกับเบอร์โทรของผู้ขายเขียนในใบเสร็จด้วยได้ไหมครับ |
| Is AppleCare+ left, and can it transfer to me? | มี AppleCare+ เหลืออยู่ไหมครับ แล้วโอนให้ผมได้ไหมครับ |

Two notes. **"เครื่องเคยแกะไหมครับ"** ("has it ever been pried open") is a genuinely different
question in Thai from "has it been repaired" — a seller who has had an independent shop inside
the machine is more likely to answer it honestly. And ask for **ใบเสร็จรับเงิน**, not
**ใบกำกับภาษี**, unless the shop is visibly a VAT-registered business: a private seller cannot
lawfully issue a tax invoice.

### The paperwork is not theatre

The original tax invoice (**ใบกำกับภาษี**) is simultaneously the only real provenance test on
an M1–M5 Mac, the thing that transfers AppleCare+, and what the police want for a theft
report. One ask, three payoffs.

### Where you buy changes your legal position more than any check

Under Civil and Commercial Code **s.1332**, a good-faith buyer who buys **in a market or from
a dealer in that kind of trade** keeps the machine even if it turns out to be stolen, unless
the true owner refunds what they paid. Buy from a **private individual** and you hand it back
with nothing.

The dealer limb needs no registration of any kind — it is about what the seller actually
trades in. The script asks you one question about this and prints the branch that applies.

**Do not assume this holds outside Thailand.** s.1332 is unusually buyer-friendly and may
invert where you are. The portable rule is not the statute — it is **name your seller**. A
remedy against the seller exists everywhere, and it needs a name you can find.

### Market risks specific to Thailand

- **MDM here is often a financing lock.** Instalment and pawn shops enrol devices so they can
  lock them on non-payment. **จำนำไอคลาวด์** — "pawning the iCloud" — is a real practice: the
  Apple Account is pledged to a lender while the owner keeps using the device.
- **AppleCare+ bought in Thailand is Thailand-only and transfers exactly once**, and needs the
  original invoice plus written notice to Apple South Asia. Apple's contract never promises
  worldwide Mac service, and reserves the right to restrict service to the country of sale.
  Coverage only pays for a battery once it is already **below 80%** of original capacity —
  Apple's AppleCare+ terms set the threshold at *"less than eighty percent (80%)"* — so a
  merely mediocre battery gets nothing from it.
- **Nobody labels their own machine a grey import.** Sellers advertise **ศูนย์ไทย** as a
  positive claim, so the *absence* of that claim is the signal, not the presence of a
  **หิ้ว** label.
- **No Thai state channel lets a buyer check whether a device is stolen.** None. The market's
  real protection is **ประกันร้าน**, a 7–90 day shop warranty covering hardware only — it
  covers no provenance risk at all.
- **No platform protects the sale.** Meta excludes local-pickup cash; Kaidee has no payment
  rail.
...
A real run, on a real machine, lightly redacted. Two faults were answered as present to show
what a finding looks like.

```
   🛑 0 high risk    💰 2 renegotiate    📝 2 note    ?? 1 not established

   💰  A port will not charge
        charge-port board ฿1,500-3,500 independent (as of 2026-08-04)
   💰  A key does not register
        top case ฿7,490 + ฿1,605 fitting (Apple TH list, effective 2023-11-01)
   📝  Remote-access software has been installed
   📝  Coverage is live, and here is what it is actually worth
   ??  Load test ran on the charger, not on battery

   Repair total, at least: ฿8,990

  Grade A at best (BKK APPLE rubric, TH) — battery 100%.
  Cosmetic condition, which you can see and this script cannot, can only
  lower that.
```

The header line is a **ceiling, not a grade**. Every criterion the script can read is
disqualifying — battery capacity, locks, repair record — while the one it cannot read
(cosmetics) can only push the grade down. So *not Grade A* is conclusive, and *is Grade A* is
unreachable by a script. The rubric is a Thai buyback operator's published one, used here as a
condition vocabulary: three of its four criteria are Apple's own thresholds and panes.

Note the `??` line. The load test ran plugged in, so the thermal half is valid and the battery
half is not, and the report says so rather than quietly counting it as a pass.

---

## Notes on a few decisions, so they are not re-litigated

**Why a script instead of a checklist.** A checklist and a script are two sources of truth for
the same thirty-odd checks, and they drift. The script is what actually ran, so the script
wins. This file explains; it is not meant to be opened in front of a seller.

**Why every command is called by an absolute path.** Because the readings can otherwise be
faked trivially. Demonstrated first-hand: a three-line shell script placed earlier on `PATH`
faked an entire hardware sheet — model, chip, RAM, serial, and Activation Lock `Disabled` —
with no privileges at all. Absolute paths defeat it, because the real binaries sit on the
sealed system volume where an unprivileged user cannot touch them. That is also why the script
reports SIP state first: **the readings are only as good as SIP being on.**

**Why the load test runs on battery.** Plugged in, the battery is never asked to supply
current, so a cell that collapses under load stays invisible. On battery the same run tests
cooling *and* the battery. It is also the only power state guaranteed to exist — the seller's
charger is a ฿1,190–3,190 accessory that may not be in the room.

**Why a still-climbing thermal curve is not graded on a MacBook Pro.** The 180-second window
was measured once, on a fanless Air — the thermally weakest configuration there is. A
fan-cooled Pro may simply saturate more slowly, and grading it would mean telling you to
renegotiate on a healthy machine using a threshold nobody has measured on that class. The
script reports the trajectory and declines to grade it. Levels 3 and 4 still grade on both,
because those are Apple's own absolute scale rather than a curve-derived threshold.
...
`check.sh` by `tools/sync-readme.sh` — do not edit it here.

<details>
<summary><strong>check.sh</strong> — click to expand, then copy all of it</summary>

<!-- BEGIN GENERATED check.sh -->
